      ...
      setData((prevState)=>{
        return prevState.map((obj)=>{
          return {...obj,videoCollection: obj.videoCollection.map((obj)=>{
            if (Number(e.target.id) === Number(obj.authorID)) {
      ...
      {(() => {
        if (viewState === 'Home') {
          ...
          return (
            <>
              <Header handleClicks={handleHomeClick}/>
              <Containers key={videoState.id} collections = {dataState[0].videoCollection} targetVideo={videoState} handleClicks={handleSubClick} handleClicksVid={handleVidClick}/>
            </>
          )
        }
      })()}
