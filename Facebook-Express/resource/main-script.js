...
let scrollCount = 3080 // captures number of scrolls in windows
...
let reelBlock = document.querySelector('.reel')
let btnRight = document.querySelector('.floating--right')
let btnLeft = document.querySelector('.floating--left')

btnRight.addEventListener('click',()=>{
    reelBlock.scrollLeft += 100
    if (reelBlock.scrollLeft !== 0) {
        document.querySelector('.floating--left').setAttribute('style','visibility: visible;')
    }
    if (reelBlock.scrollLeft >= 199) {
        document.querySelector('.floating--right').setAttribute('style','visibility: hidden;')
    }
    console.log(`right click ${reelBlock.scrollLeft}`)
})

btnLeft.addEventListener('click',()=>{
    reelBlock.scrollLeft -= 100
    if (reelBlock.scrollLeft === 0) {
        document.querySelector('.floating--left').setAttribute('style','visibility: hidden;')
    }
    if (reelBlock.scrollLeft <= 199) {
        document.querySelector('.floating--right').setAttribute('style','visibility: visible;')
    }
    console.log(`left click ${reelBlock.scrollLeft}`)
})
